...
static NSString *const kReceiverAppID = @"D19C49A0"; //Update to your app id to host your own receiver
...
- (IBAction)joinTable:(id)sender {
  NSLog(@"sending text %@", [self.messageTextField text]);
  ...
  if (!self.deviceManager || !self.deviceManager.isConnected) {
    ...
    // No name specified
    if ([self.messageTextField.text length] == 0 || self.messageTextField.text == nil || [self.messageTextField.text isEqual:@""] == YES) {
        UIAlertView *alert =
        [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"No player name", nil)
                                   message:NSLocalizedString(@"Please choose a name before joining", nil)
                                  delegate:nil
                         cancelButtonTitle:NSLocalizedString(@"OK", nil)
                         otherButtonTitles:nil];
        [alert show];
    ...
    [self.textChannel sendTextMessage:[self dictToJSON:@{@"name":[self.messageTextField text]}]];
    [self performSegueWithIdentifier:@"connectGame" sender:self];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    HTGCWaitViewController *waitViewController = segue.destinationViewController;
    
    NSLog(@"prepareForSegue: %@", segue.identifier);
    if([segue.identifier isEqualToString:@"connectGame"]) {
        [self.textChannel setDelegate:segue.destinationViewController];
        waitViewController.selectedDevice = self.selectedDevice;
        waitViewController.textChannel = self.textChannel;
    }
}
